...
from universal_mcp_retell.app import RetellApp

@pytest.fixture
def app_instance():
    """Provides a RetellApp instance for tests."""
    mock_integration = MagicMock()
    mock_integration.get_credentials.return_value = {"access_token": "dummy_access_token"}

    return RetellApp(integration=mock_integration)

def test_universal_mcp_retell_app_initialization(app_instance):
    """
    Test that the RetellApp instance is initialized correctly with a name.
    """
    ...
    assert app_instance.name == "retell", "RetellApp instance has unexpected name."


def test_universal_mcp_retell_tool_docstrings_format(app_instance):
    """
    Test that each tool method in RetellApp has a well-formatted docstring,
...
def test_universal_mcp_retell_tools_are_callable(app_instance):
    """
    Test that each tool method returned by list_tools in RetellApp is callable.
    """
    tools = app_instance.list_tools()
    assert isinstance(tools, list), "list_tools() should return a list."
